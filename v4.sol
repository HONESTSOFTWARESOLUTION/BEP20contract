...
contract version2 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
       ...
       uint8 totFee  = _swalletFee + _sliquidityFee  + _sprofitsFee; 
       uint256 spentAmount = 0;
       uint256 totSpentAmount = 0;

       if(_swalletFee != 0){
            spentAmount = contractTokenBalance.div(totFee).mul(_swalletFee);
       ...
       if(_sprofitsFee != 0){
            spentAmount = contractTokenBalance.div(totFee).mul(_sprofitsFee);
            uint256 initialBalance = address(this).balance;
            swapTokensForBNB(spentAmount);
            uint256 newBalance = address(this).balance.sub(initialBalance);
    ...
    function _transfers(
        ...
        if(takeFee && (buyFee+sellFee) > 0){  
            uint256 taxAmount = amount.mul(sellFee).div(100);           
            if(from == pcsV2Pair){
